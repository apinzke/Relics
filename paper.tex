...
\begin{eqnarray}
  \label{eq:f_e}
  f_\e(\gamma_\e) &\approx& \frac{f_\rmn{inj,e}(\gamma_\e)}{\alpha-1}
...
approximation in Eqn.~(\ref{eq:f_e}) is valid as long as
...
populations, each determined by Eqn.~\ref{eq:f_e} but with a
different spectral index and injected spectra.


\subsection{Synchrotron emission}
WORK UNDER PROGRESS During the merging of two clusters, both
turbulence flows and merging shocks are induced. These processes are
thought to accelerate CRs to high energy, where especially the
distribution function of preexisting CRs is enhanced. In this section
we use the smooth CR electron distribution derived from a simulated
merging cluster in the previous section, boost it using the details of
the shock, and then finally derive the radio synchrotron emission.

We fit the steady state CR electron distribution function in
Fig.~\ref{fig:e_spec} with two power-laws
\begin{equation}
  f_\rmn{e,fit}(\gamma_\e) = C_1\,\gamma_\e^{-\alpha_1}
\theta(\gamma_\rmn{break}-\gamma_\e) 
+ C_2\,\gamma_\e^{-\alpha_2}
\theta(\gamma_\e-\gamma_\rmn{break})\,. 
\label{eq:f_fit}
\end{equation}
...
\Gamma\left(\frac{3 \alpha_i-1}{12}\right)\,\Gamma\left(\frac{3 \alpha_i+19}{12}\right)\frac{3^\frac{\alpha_i}{2}}{\alpha_i+1}\,\left(\frac{\nu}{\nu_c}\right)^{\frac{1-\alpha}{2}}\,,
\end{equation}
...
\begin{equation}
  f_2(\gae) = q \gae^{-q}\int_{\gamma_\rmn{inj}}^\gamma \gamma'^{q-1}
   f_\rmn{e,fit}(\gamma')\dd \gamma'\,.
\end{equation}
...
\begin{equation}
\gamma_\rmn{inj} \approx 1.17 \frac{u_2}{c}\left(1+\frac{1.07}{\epsilon_B}\right)
...
$\sigma=[(\gamma_\rmn{ad}+1)M^2]/[(\gamma_\rmn{ad}-1)M^2+2]$, where
adiabatic index $\gamma_\rmn{ad}=5/3$, $M_A=M c_s/v_A$, and $ c_s$ the
