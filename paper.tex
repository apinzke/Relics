...
into the inter-stellar medium from supernova remnants. Although, note
that the contribution from the CR electrons that are accelerated in
supernova remnants is expected to be negligible compared to the CR
electron population the ICM, because of the much shorter cooling times
associated with the remnants. IS THIS TRUE? \del{(see Pinzke, Oh, and
  Pfrommer, in prep. for a discussion of this topic).}
...
calculating synchrotron emission from galaxy clusters that are
undergoing a merger at low redshifts.

Our scheme is summarized, in order, by the following bullet points:
\begin{itemize}
\item derive CR proton distribution function from each simulation
  output at different time $t$
\item calculate injected CR proton population between snapshots 
\item convert injected protons to injected electrons
\item evolve each injected CR electron population to a later time
  $\tend$ while accounting for losses (Coulomb, inverse Compton, and
  adiabatic)
...
The CR protons in are represented by an isotropic one-dimensional
power-law distribution function \footnote{The three-dimensional
  distribution function is given by $4\pi \int f(\gam)\, \gam^2\, \dd
  \gam$.}  given by
\begin{equation}
  \label{eq:f_p}
...
$m_\p$. Here $q$ is the proton mass normalized momentum cutoff,
$\alpha$ the spectral index, and $C$ the normalization of the
distribution function in units of density.

To improve the numerical efficiency of our simulations the CR proton
...
the CR protons because of the different injection efficiencies and the
timescales of loss processes. Instead we first need to derive the
injected CR proton distribution function between each snapshot.  The
injected distribution function is calculated as the change in CR
normalization between a snapshot at time $t$ and an earlier time
$t-\Delta t$:
\begin{eqnarray}
  \label{eq:finj}
...
region between $\gamma \sim (3-5\times10^2)$ the typical cooling times
are small enough to be resolved by our simulations.
...
related to the injected CR protons through
\begin{eqnarray}
  \label{eq:f_inj_e}
...
and evolving it to a later time $t_f$ \citep[for further details
  see][]{1999ApJ...520..529S}. The loss of energy for each particle is
described by
\begin{equation}
\label{eq:d_cool}
...
momentum cut-off between time $t_i$ and the later time $t_f$ by a discrete sum;
\begin{eqnarray}
...
\end{eqnarray}
where $j$ denotes the summation index that run over all snapshots
between time $t_i$ and the later time $t_f$. Here we have approximated
the low momentum cut-off between two snapshots by the mean given by
$\gam_\low(\Delta t_j) = \Delta t_j \left[b_\rmn{C}(t_{j-1})+
  b_\rmn{C}(t_j)\right]/2$, where $\Delta t_j = t_j - t_{j-1}$.

We now continue with the IC losses that are given by
\begin{equation}
  \label{eq:b_IC}
  b_\IC(\gam,z) = b_\IC \gam^2 (1+z)^4
  = \frac{4}{3}\frac{\sigma_\rmn{T}}{m_\e c} U_\CMB \gam^2\,.
\end{equation}
Here $\sigma_\rmn{T}= 8\pi e^4/3(m_\e c^2)^2$ is the Thomson cross
section and $U_\CMB=B_\CMB^2/8\pi$ is the energy density of the CMB at redshift
...
\end{equation}
When a time $(t_f-t_i)$ has elapsed, all the electrons with momentum
above $\gam_\ma$ have thermalized. We derive $\gam_\ma$ by integrating
Eqn.~(\ref{eq:b_IC_evolu}) from the redshift $z_i=z(t_i)$ where the
electrons are injected to a later time $z_f=z(t_f)$:
\begin{equation}
  \label{eq:b_max}
...
\left.\frac{\partial\gam_i}{\partial\gam_f}\right|_{t_f} &=& 
\frac{\gam_\ma^2}{\left(\gam_\ma-\gam_f\right)^2}\,.
\end{eqnarray}
Here we have used that $\Delta\gam_\IC=-\gam_\low$ and
$\Delta\gam_\rmn{C}=\frac{-\gam_f^2}{\gam_\ma-\gam_f}$. The total
electron spectrum is derived from the sum of all individually cooled
injected spectra, starting from the time of injection $t_i$ until a
later time $t_f$,
\begin{eqnarray}
  \label{eq:f_sum}
...
takes care of the adiabatic gains and losses (see Eqns.\ref{eq:q0} and
\ref{eq:C0}), where $\rho_0=1$ is a reference comoving density in
GADGET. \del{Also note that the electrons injected at time $t=t_i$
  have not had the time to cool yet.}

We model the final CR electron \AP{distribution} function as a
superposition of two CR populations, each determined by
Eqn.~\ref{eq:f_sum}, but with a different spectral index
($\alpha=[2.1, 2.3]$) and injected normalization $C$ derived from the
simulations. The spectral index bins includes all CR electrons with
$\alpha \leq 2.4$, which corresponds to a $\mach > 4.7$. For weaker
shocks we expect the functional form of the injection efficiency of the
electrons to greatly deviate from the protons due to the injection
problem that we discussed in Sect.~\ref{sect:intro}. In
Figure~\ref{fig:e_spec_tests} we show how different cuts in $\alpha$
impact the CR electron distribution function. 

\del{CR electron distribution function is derived from summing up 1000
  SPH particles. Show figure with convergence study.}


\subsection{CR electrons -- reacceleration}
\label{sect:reacc}
Accretion and merger events in galaxy clusters are often very violent,
and large amounts of gravitational energy is being dissipated in the
form of radiation, increased temperature, turbulent flows and
shocks. The shock accelerated electrons emit radio synchrotron
emission that has been observed in close to 50 clusters, and in
addition there are merger shocks that only are observed in X-rays
without any counterpart in radio. The typical size of the shocked gas
that emit in radio (radio relics) is of the order of a Mpc with a
thickness of the order 100~kpc. Usually these relics are detected in
the cluster outskirts where the gas can reach keV temperatures which
result in typically weak shock ($\mach\lesssim 5$). Since the weak
shocks are very inefficient in accelerating thermal electrons to high
energies, we neglect this contribution in this work, and only consider
the reaccelerated pre-existing CR electrons.
...
\begin{eqnarray}
  f_\rmn{re-acc,e}(\gae) &=& q\,\gae^{-q} C_\rmn{re-acc}\,,\qquad\rmn{where}\\
C_\rmn{re-acc}&=& \int_{\gam_\rmn{inj}}^{\gam_\rmn{cut}} \gam'^{q-1}
  f_\rmn{tot,e}(\gam')\dd \gam'\,.
\end{eqnarray}
The integral is cut above the momentum $\gam_\rmn{cut} \sim
5\times10^2$ that roughly corresponds to the momentum where our
formalism for the CR electrons break down due to insufficient time
resolution. The lowest momentum boundary above which particles can
cross the shock is given by
\begin{equation}
  \gam_\rmn{inj} \approx 1.17 \frac{u_2}{c}\left(1+\frac{1.07}{\epsilon_B}\right)
  \left(\frac{\mach}{3}\right)^{0.1}\,,
\end{equation}
and the one dimensional spectral index of the reaccelerated CR
electrons is derived from
\begin{equation}
  q = \frac{\left(u_1-v_A\right)+2u_2}{u_1-v_A-u_2} = 
  \frac{\sigma\left(1-\mach_A^{-1}\right)+2}{\sigma-1-\sigma \mach_A^{-1}}\,.
\end{equation}
Here $u_1$ and $u_2$ are the speed in the shock rest frame of the flow
upstream and downstream, respectively. The compression ratio is given
by $\sigma=u_1/u_2=\rho_2/\rho_1$, and $\mach_A=u_1/v_A$ is the
upstream Alfv̩n Mach number with velocity
...
where the adiabatic index $\gam_\rmn{ad}=5/3$, $\mach_A=\mach
c_s/v_A$, and $c_s$ is the upstream sound speed. We note that for a
typical merger with $c_s \sim 500\,\rmn{km}/\rmn{s}$, $\rho \sim 500
\rho_\rmn{crit} \Omega_\rmn{b}$ and $\mach \sim 3.5$, the spectral
index of the CR electrons after reacceleration is given by $q \sim
2.6$.


\subsection{radio synchrotron emission}
 We calculate the radio synchrotron emissivity of the reaccelerated CR
 electrons that follow a power-law distribution by using the formalism
 derived in \cite{1979rpa..book.....R}:
\begin{equation}
J(\nu) \approx \frac{3^\frac{q}{2}\,e^2\,\nu_c}{c(q+1)}\,
C_\rmn{re-acc}
\Gamma\left(\frac{3q-1}{12}\right)\,
\Gamma\left(\frac{3q+19}{12}\right)\,
\left(\frac{\nu}{\nu_c}\right)^{\frac{1-q}{2}}\,,
\end{equation}
...
\begin{equation}
  P(\nu) = \rmn{Volume} \times J(\nu)\,.
\end{equation}
For a typical relic, $q \sim 2.6$ (see Sect.~\ref{sect:reacc}), $z
\sim 0.1$, largest linear size (LLS) $\sim 1 \rmn{Mpc}$, thickness
$\sim 0.1 \rmn{Mpc}$, size along line-of-sight $\sim 0.5 \rmn{Mpc}$
(total volume $\sim 0.05\,\rmn{Mpc}^3$), we find that the flux
$P(1.4\,\rmn{PHz})=4\times10^{23}$~W/Hz which is in the lower end of
observations of radio relics. Although, note that this flux estimate
is derived using the median CR electron distribution function of 14
clusters where there is a spread of about a factor 50, that can easily
account for the more luminous radio relics.

\del{In addition we assume that the average magnetic
...
\begin{figure}
  ...
  \caption{Cosmic ray electron spectra in cluster outskirts at
    ...
    spectra from all clusters. Note that the CR spectra peaks at
    $\gamma \sim 30$ for all clusters (\AP{check cooling times}, and
    that there is about a factor 50 difference in the spectra between
    a large merging cluster and a small cool core cluster in the
    regime $\gamma_\e \sim (3,300)$. \label{fig:e_spec_all}}
\end{figure}
...
\begin{figure*}
\begin{minipage}{2.0\columnwidth}
  \includegraphics[width=0.33\columnwidth]{./figures/CRspec.alphaCheck.eps}
  \includegraphics[width=0.33\columnwidth]{./figures/CRspec.snapCheck.eps}
  \includegraphics[width=0.33\columnwidth]{./figures/CRspec.partCheck.eps}
  \caption{Testing the robustness of the CR electron spectra. Left
    ...
    without a cut. Middle panel shows the effect of time-resolution on
    ...
    between snapshots. Right panel show the converegence in number of
    particles used of the CR electron spectra for three representative
    clusters. We find that 1000 particles is enough to represent the
    CRs within the (1.3-1.5)$\rvir$ region of the clusters. \AP{Do we
      want to put these figures in an
      appendix?} \label{fig:e_spec_tests}}
\end{minipage}
\end{figure*}

\begin{figure}
  \includegraphics[width=1.0\columnwidth]{./figures/CRespecRadial.g72a.2Rv15000p.a24.full.140.v21.eps}
  \caption{Spatial dependence of the cosmic ray electron spectra at
    ...
    the CR electron spectra for different radial bins. \AP{Add other
      clusters. We might want to put this type of figure in the second
      paper!?!}\label{fig:e_spec_spatial}}
\end{figure}

\begin{figure}
  \includegraphics[width=1.0\columnwidth]{./figures/FluxMach.1.4Rv.a24.full.020.v21.eps}
  \caption{Flux from radio relics and the associated Mach numbers. We
    show the observed radio relic fluxes at 1.4 GHz (diamonds) and
    their Mach numbers as derived from radio spectral index. We
    contrast the observations with the predicted fluxes (X) where the
    error bars corresponds to the 68 percentiles of the volume
    weighted CR electron spectra across our cluster sample. The dashed
    line show the expected flux of a typical relic where we only
    change the Mach number. \AP{THREE HIGH MACH NUMBER RELICS NO SPECTRAL
    INDEX, NEED TO LOOK IN LITERATURE. MIGHT WANT TO INCLUDE LOFAR
    PREDICTIONS AS WELL?}\label{fig:Fluc_Mach}}
\end{figure}
